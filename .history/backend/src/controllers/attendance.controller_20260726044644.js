...
/** GET /api/attendance/summary?month= — HR/Admin: every employee's attendance summary for a month ('All Employees' view). */
const allEmployeesSummary = asyncHandler(async (req, res) => {
  const month = req.query.month || new Date().toISOString().slice(0, 7);
  const [from, to] = monthRange(month);
  ...
  res.json({ success: true, data: { month, employees: result.rows } });
});

module.exports = { myAttendance, listAttendance, markAttendance, markBulk, allEmployeesSummary };