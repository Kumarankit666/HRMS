  ...
  upload(req, res, async (err) => {
    try {
      ...
      await createNotification({
        recipientRole: 'HR', title: 'New Document Uploaded',
        message: `A ${docType} document was uploaded and needs verification.`, link: '/app/documents'
      });
...
/** GET /api/documents?employeeId= — HR/Admin views one employee's documents, or all if employeeId is omitted. */
const listDocuments = asyncHandler(async (req, res) => {
  const { employeeId } = req.query;

  const sql = employeeId
    ? `SELECT d.id, d.doc_type, d.file_name, d.file_path, d.verified_status, d.uploaded_at, e.employee_code, e.full_name
       FROM employee_documents d JOIN employees e ON e.id = d.employee_id
       WHERE d.employee_id = $1 ORDER BY d.uploaded_at DESC`
    : `SELECT d.id, d.doc_type, d.file_name, d.file_path, d.verified_status, d.uploaded_at, e.employee_code, e.full_name
       FROM employee_documents d JOIN employees e ON e.id = d.employee_id
