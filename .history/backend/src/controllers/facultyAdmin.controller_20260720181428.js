...
const addFaculty = asyncHandler(async (req, res) => {
  upload(req, res, async (err) => {
    ...
    const { fullName, role, subject, displayOrder } = req.body;
    if (!fullName) {
    ...
    const result = await query(
      `INSERT INTO faculty_members (full_name, role, subject, photo_path, display_order, created_by)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`,
      [fullName, role || null, subject || null, photoPath, displayOrder || 1, req.user.id]
    );
...
const updateFaculty = asyncHandler(async (req, res) => {
  upload(req, res, async (err) => {
    if (err) return res.status(400).json({ success: false, message: err.message || 'Upload failed.' });
    const { id } = req.params;
    const { fullName, role, subject, displayOrder, active } = req.body;
