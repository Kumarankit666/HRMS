...
const generate = asyncHandler(async (req, res) => {
  ...
  if (toEmail) {
    await sendMail({
      ...
      text: `Hi ${emp.full_name},\n\nCongratulations! Your offer letter is ready.\n\nLog in to view it and sign your consent/policy documents:\n${process.env.FRONTEND_URL || 'http://localhost:5173'}/login\n\n— ${SYSTEM.APP_NAME}`
    });
  }

  await logAudit(req.user.id, 'GENERATE_OFFER_LETTER', 'OfferLetter', employeeId);

  // Assign every currently-active policy document to this employee for signing.
  try {
    const activePolicies = await query(`SELECT id FROM policy_documents WHERE active = true`);
    for (const p of activePolicies.rows) {
      await query(
        `INSERT INTO policy_acknowledgements (employee_id, policy_document_id) VALUES ($1,$2)
         ON CONFLICT (employee_id, policy_document_id) DO NOTHING`,
        [employeeId, p.id]
      );
    }
  } catch (e) {
    console.error('Policy acknowledgement assignment failed (non-fatal):', e.message);
  }
