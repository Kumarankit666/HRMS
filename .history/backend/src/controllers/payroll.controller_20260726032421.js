...
const generatePayslip = asyncHandler(async (req, res) => {
  ...
  // Aadhaar/PAN and bank fallback come from the onboarding form if the employees table columns are blank.
  ...
  const incentiveVal = incentive !== undefined && incentive !== '' ? Number(incentive) : Number(s.incentive || 0);
  ...
  await query(
    ...
    [employeeId, month, year, totalEarning, totalDeduction, tdsVal, netPayable, publicPath, req.user.id,
      incentiveVal, arrearVal, travellingVal, loanAdvanceVal, lopDaysNum, lopAmount, daysInMonth]
  );

  // No email is sent for payslips — HR/employee view or download it from the HRMS directly.
  await logAudit(req.user.id, 'GENERATE_PAYSLIP', 'Payroll', `${employeeId} ${month}/${year}`);
  res.json({ success: true, message: 'Payslip generated.', data: { fileUrl: publicPath, net: netPayable } });
});
