...
const addTopper = asyncHandler(async (req, res, next) => {
  upload(req, res, async (err) => {
    try {
      ...
      const { studentName, examName, examYear, rankAchieved, currentStatus, displayOrder } = req.body;
      ...
      const result = await query(
        `INSERT INTO topper_students (student_name, exam_name, exam_year, rank_achieved, current_status, photo_path, display_order, created_by)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
        [studentName, examName, examYear, rankAchieved, currentStatus || null, photoPath, order, req.user.id]
      );
...
const updateTopper = asyncHandler(async (req, res, next) => {
  upload(req, res, async (err) => {
    try {
      if (err) return res.status(400).json({ success: false, message: err.message || 'Upload failed.' });
      const { id } = req.params;
      const { studentName, examName, examYear, rankAchieved, currentStatus, displayOrder, active } = req.body;
