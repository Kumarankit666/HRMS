              ...
              {managerItems.length === 0 ? <p className="text-muted mb-0">Nothing pending.</p> : (
                <div className="table-wrap">
                  <table className="hrms-table">
                    <thead><tr><th>Employee</th><th>Date</th><th>Requested Status</th><th>Reason</th><th></th></tr></thead>
                    <tbody>
                      {managerItems.map((r) => (
                        <tr key={r.id}>
          ...
          {isAdminOrHr && (
            <div className="card">
              ...
              {hrItems.length === 0 ? <p className="text-muted mb-0">Nothing pending.</p> : (
                <div className="table-wrap">
                  <table className="hrms-table">
                    <thead><tr><th>Employee</th><th>Date</th><th>Requested Status</th><th>Manager Remark</th><th></th></tr></thead>
                    <tbody>
                      {hrItems.map((r) => (
                        <tr key={r.id}>
                          <td>{r.full_name} <span className="text-muted">({r.employee_code})</span></td>
