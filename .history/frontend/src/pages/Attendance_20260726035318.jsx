...
export default function Attendance() {
  ...
  const [reqForm, setReqForm] = useState({ requestDate: '', requestedStatus: 'Present', reason: '' });
  ...
  async function handleRequestSubmit(e) {
    ...
    try {
      ...
      toast.success(res.data.message);
      setReqForm({ requestDate: '', requestedStatus: 'Present', reason: '' });
      loadMyRequests();
    } catch (err) {
      toast.error(apiErrorMessage(err));
    } finally {
        ...
        <div className="grid-2" style={{ marginTop: 20 }}>
          ...
          <div className="card">
            ...
            {myRequests.length === 0 ? <p className="text-muted mb-0">No requests submitted yet.</p> : (
              <div className="table-wrap">
                <table className="hrms-table">
                  <thead><tr><th>Date</th><th>Requested</th><th>Status</th></tr></thead>
                  <tbody>
                    {myRequests.map((r) => (
                      <tr key={r.id}>
