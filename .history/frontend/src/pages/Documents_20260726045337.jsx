...
export default function Documents() {
  ...
  useEffect(() => {
    if (isAdminOrHr && selectedEmployee === '__ALL__') loadAll();
  ...
  async function verify(id, status) {
    try {
      ...
      toast.success(res.data.message);
      selectedEmployee === '__ALL__' ? loadAll() : loadFor(selectedEmployee);
    } catch (err) {
      toast.error(apiErrorMessage(err));
    }
  }
  ...
  async function remove(id) {
    ...
    try {
      ...
      toast.success(res.data.message);
      if (!isAdminOrHr) loadMine();
      else selectedEmployee === '__ALL__' ? loadAll() : loadFor(selectedEmployee);
    } catch (err) {
      toast.error(apiErrorMessage(err));
    }
  }
          ...
          {docs.length === 0 ? <p className="text-muted mb-0">No documents uploaded yet.</p> : (
            <div className="table-wrap">
              <table className="hrms-table">
                <thead><tr>{selectedEmployee === '__ALL__' && <th>Employee</th>}<th>Type</th><th>File</th><th>Status</th><th>Uploaded</th><th></th></tr></thead>
                <tbody>
                  {docs.map((d) => (
                    <tr key={d.id}>
                      {selectedEmployee === '__ALL__' && <td>{d.full_name} <span className="text-muted">({d.employee_code})</span></td>}
                      <td>{d.doc_type}</td>
