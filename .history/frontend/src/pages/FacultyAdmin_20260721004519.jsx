...
function FacultyModal({ existing, onClose, onDone }) {
  ...
  const [experienceYears, setExperienceYears] = useState(existing?.experience_years || '');
    ...
    <Modal title={existing ? 'Edit Faculty Member' : 'Add Faculty Member'} onClose={onClose} footer={
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="grid-2">
          ...
          <div className="field"><label>Highest Qualification</label><input value={qualification} onChange={(e) => setQualification(e.target.value)} placeholder="e.g. M.Sc, Ph.D" /></div>
          <div className="field"><label>Years of Experience</label><input type="number" step="0.5" value={experienceYears} onChange={(e) => setExperienceYears(e.target.value)} placeholder="e.g. 8" /></div>
        </div>
