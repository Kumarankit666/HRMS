          ...
          <div className="pub-grid-3">
            {faculty.map((f) => (
              <div className="pub-card pub-faculty-card" key={f.id}>
                ...
                <p className="text-muted" style={{ fontSize: '0.85rem', marginBottom: 4 }}>{f.subject}</p>

                {(f.qualification || f.experience_years) && (
                  <>
                    <p className="pub-faculty-hint">Hover for details</p>
                    <div className="pub-faculty-meta">
                      {f.qualification && (
                        <span className="pub-faculty-badge">🎓 {f.qualification}</span>
                      )}
                      {f.experience_years && (
                        <span className="pub-faculty-badge">💼 {f.experience_years} experience</span>
                      )}
                    </div>
                  </>
                )}
