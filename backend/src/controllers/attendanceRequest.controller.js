...
const createRequest = asyncHandler(async (req, res) => {
  const { requestDate, requestedStatus, reason, inTime, outTime } = req.body;
  ...
  const result = await query(
    `INSERT INTO attendance_requests (employee_id, request_date, requested_status, reason, in_time, out_time, manager_status, manager_remark)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
    [req.user.id, requestDate, requestedStatus, reason || null, inTime || null, outTime || null,
  ...
  if (hasManager) {
    await createNotification({
      recipientId: empRes.rows[0].reporting_manager_id, title: 'Attendance Correction Request',
      message: `${empRes.rows[0].full_name} requested an attendance correction for ${requestDate}.`, link: '/app/attendance-approvals'
    });
  } else {
    await createNotification({
      recipientRole: 'HR', title: 'Attendance Correction Request',
      message: `${empRes.rows[0].full_name} requested an attendance correction for ${requestDate}.`, link: '/app/attendance-approvals'
    });
  }
...
const myRequests = asyncHandler(async (req, res) => {
  const listRes = await query(
    `SELECT id, request_date, requested_status, reason, in_time, out_time, manager_status, manager_remark, hr_status, hr_remark, created_at
...
const managerPending = asyncHandler(async (req, res) => {
  ...
  const result = await query(
    `SELECT ar.id, ar.request_date, ar.requested_status, ar.reason, ar.in_time, ar.out_time, ar.created_at,
...
const hrPending = asyncHandler(async (req, res) => {
  const result = await query(
    `SELECT ar.id, ar.request_date, ar.requested_status, ar.reason, ar.in_time, ar.out_time, ar.manager_remark, ar.created_at,
            e.employee_code, e.full_name
     FROM attendance_requests ar JOIN employees e ON e.id = ar.employee_id
...
const hrDecision = asyncHandler(async (req, res) => {
  ...
  if (decision === 'Approved') {
    await query(
      `INSERT INTO attendance (employee_id, date, status, in_time, out_time, updated_by)
       VALUES ($1,$2,$3,$4,$5,$6)
       ON CONFLICT (employee_id, date) DO UPDATE SET status = $3, in_time = $4, out_time = $5, updated_by = $6, updated_at = now()`,
      [record.employee_id, record.request_date, record.requested_status, record.in_time || null, record.out_time || null, req.user.id]
    );
  }
