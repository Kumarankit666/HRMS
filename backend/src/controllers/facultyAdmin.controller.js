...
const addFaculty = asyncHandler(async (req, res, next) => {
  upload(req, res, async (err) => {
    try {
      ...
      const expYears = experienceYears === '' || experienceYears === undefined ? null : experienceYears.trim();
      ...
      const result = await query(
        ...
        [fullName, role || null, subject || null, photoPath, order, qualification || null, expYears, req.user.id]
      );
...
const updateFaculty = asyncHandler(async (req, res, next) => {
  upload(req, res, async (err) => {
    try {
      ...
      if (role !== undefined) { sets.push(`role = $${i++}`); values.push(role || null); }
      if (subject !== undefined) { sets.push(`subject = $${i++}`); values.push(subject || null); }
      if (displayOrder !== undefined) { sets.push(`display_order = $${i++}`); values.push(displayOrder === '' ? 1 : displayOrder); }
      if (qualification !== undefined) { sets.push(`qualification = $${i++}`); values.push(qualification || null); }
      if (experienceYears !== undefined) { sets.push(`experience_years = $${i++}`); values.push(experienceYears === '' ? null : experienceYears.trim()); }
