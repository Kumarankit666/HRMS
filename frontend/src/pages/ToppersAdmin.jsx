      ...
      <div className="card">
        <div className="table-wrap">
          <table className="hrms-table">
            <thead><tr><th></th><th>Student</th><th>Exam</th><th>Year</th><th>Rank</th><th>Marks</th><th>Status</th><th></th></tr></thead>
            <tbody>
              {list.length === 0 ? (
                <tr><td colSpan={8} className="text-muted">No topper students yet.</td></tr>
              ) : list.map((t) => (
                <tr key={t.id}>
                  <td>
                    {t.photo_path ? (
...
function TopperModal({ existing, onClose, onDone }) {
  ...
  const [marksObtained, setMarksObtained] = useState(existing?.marks_obtained || '');
    ...
    <Modal title={existing ? 'Edit Topper Student' : 'Add Topper Student'} onClose={onClose} footer={
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="grid-2">
          ...
          <div className="field"><label>Marks Obtained</label><input value={marksObtained} onChange={(e) => setMarksObtained(e.target.value)} placeholder="e.g. 455/500" /></div>
